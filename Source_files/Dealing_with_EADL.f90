...
!***************************************************************
! This module contains subroutines for dealing with EPICS (EADL and EPDL) databases
...
subroutine check_atomic_parameters(NumPar, Target_atoms, N_at, cur_shl, shl, Error_message, read_well) ! from module 'Dealing_with_EADL'
   ...
   call Check_EPICS_files(NumPar%path_sep, File_name_EADL, File_name_EPDL97, INFO) ! check if these database-files exist
   if (INFO .EQ. 0) then ! files are there, reading them:
      if (present(cur_shl)) then ! for this shell only:
         ...
         if ((Target_atoms(N_at)%Shl_num(cur_shl) .GE. 63) .OR. (.not.NumPar%include_photons)) then
            ...
            if (Target_atoms(N_at)%Radiat(cur_shl) < 1.0d-6) then ! infinit time, no decay
               Target_atoms(N_at)%Radiat(cur_shl)=1.1d35   ! [fs]
            else
...
subroutine get_photon_cross_section_EPDL(path_sep, Ele, Target_atoms, Nat, Shl_dsgntr, Nshl, Matter, Sigma)
    ...
    File_name = trim(adjustl(Folder_name))//trim(adjustl(path_sep))//trim(adjustl(m_EPDL_file))
...
subroutine READ_EPDL_TYPE_FILE_real(FN2, File_name, Z_needed, C_needed, I_needed, S_needed, Photoabs_sigma, Eph, Shl_design, Last_dsgtr, Photoabs_sigma_tot)
   ...
   do while (Z .NE. Z_needed)
      ...
      if (Z .GE. 100) print*, 'The element is not found in the EPDL database...'
      if (Z .GE. 100) exit
   enddo
...
subroutine Check_EPICS_files(path_sep, File_name, File_name2, INFO)
    ...
    File_name = trim(adjustl(Folder_name))//trim(adjustl(path_sep))//trim(adjustl(m_EADL_file))
    !File_name2 = trim(adjustl(Folder_name))//trim(adjustl(path_sep))//'epdl97.all'
    File_name2 = trim(adjustl(Folder_name))//trim(adjustl(path_sep))//trim(adjustl(m_EPDL_file))
...
end subroutine Check_EPICS_files
